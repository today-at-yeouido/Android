...
class GetRecommendBillUseCase @Inject constructor(
    private val repository: GetBillRepository,
    private val checkLoginUseCase: CheckLoginUseCase
){
    operator fun invoke(): Flow<Resource<List<RecommendBillDto>>> = flow {
        emit(Resource.Loading())
        try{
            val response = repository.getBillUserRecommended()
            when (response.code()) {
                200 -> {
